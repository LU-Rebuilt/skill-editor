...
static BehaviorSchema& schema() {
    ...
    if (!path.empty()) {
        std::ifstream f(path);
        if (f.is_open()) {
            json_text.assign(std::istreambuf_iterator<char>(f),
    ...
    if (json_text.empty()) {
        json_text.assign(EMBEDDED_BEHAVIOR_SCHEMA, EMBEDDED_BEHAVIOR_SCHEMA_LEN);
        // Show warning (deferred to avoid calling Qt before QApplication exists)
        static bool warned = false;
        if (!warned) {
            warned = true;
            QTimer::singleShot(0, []() {
                QMessageBox::warning(nullptr, "Schema Not Found",
                    "behavior_schema.json not found next to the executable.\n\n"
                    "Falling back to the built-in default schema.\n"
                    "To use a custom schema, place behavior_schema.json\n"
                    "in the same directory as the skill_editor executable.");
            });
        }
    }
