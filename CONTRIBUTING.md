...
```
feat: add object skills editor panel
feat(object-skills): castOnType dropdown with RE'd enum names
fix: file dialog filter allowing non-sqlite files
refactor(db): split save_parameter into sqlite and memory paths
docs: add contributing guidelines
ci: add conventional commit linting
build: link lu_assets for FDB support
```
...
For local development with a local tool-common checkout:

```bash
cmake -B build -DFETCHCONTENT_SOURCE_DIR_TOOL_COMMON=/path/to/tool-common
```
